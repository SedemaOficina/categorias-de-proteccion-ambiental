...
Última revisión: 13 de septiembre de 2026 (noche). **Producción (Cloudflare): v64 (`sia-v35-2026-09-13l`). Repositorio local: v66 (`…13n`).**
...
### A1b · Publicar v66 🟠
Commit desde GitHub Desktop: `app.js`, `sw.js`, `_headers`, `CLAUDE.md` y la carpeta `pendientes/`. Luego «Purge Everything» y comprobar que `sw.js` diga `…13n`. Cambios: mapa ampliado de la ficha sin controles encimados (el lienzo sale a `<body>`; `position:fixed` no escapaba del `transform` de la hoja), `frame-ancestors 'self'`, `charset=utf-8` en `.js`/`.css`. **Probar en el iPhone:** Ampliar desde la ficha cubre toda la pantalla, sin la «×» ni el asa de la hoja encima; al salir, el mapa vuelve a su lugar en la ficha.
