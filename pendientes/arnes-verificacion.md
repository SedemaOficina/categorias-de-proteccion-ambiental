...
## 7. Producción
El contenedor no alcanza `sia.contactoverde.com` (Cloudflare Access) ni `github.io` (proxy). Lo que se verifica en producción se hace desde el Chrome del usuario (Claude in Chrome): `sw.js` muestra el `CACHE_VERSION` publicado; `getRegistrations()` y `caches.keys()` en consola dicen qué versión sirve el navegador; desde v69 el propio pie del tablero muestra «Versión del tablero». Ojo (corregido el 14-sep-2026, A11): el `ERR_FAILED` en la URL de retorno del login (`/cdn-cgi/access/authorized?…`) NO era de la extensión, sino del Service Worker anterior a v74, que interceptaba esa navegación y respondía el `index.html` cacheado. Desde v74 nada de `/cdn-cgi/` pasa por el SW (`acceso.mjs` paso 4 lo comprueba). Si un navegador conserva un SW anterior a v74, un «Unregister» en DevTools → Application → Service Workers lo resuelve.
