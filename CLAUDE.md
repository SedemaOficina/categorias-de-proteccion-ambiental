...
- **Resultado de «¿Dónde estoy?»:** una plantilla para los tres casos (dentro / Suelo de Conservación / sin cobertura); el «×» y el botón **Constancia** viven en la fila del encabezado. `compartirConstancia()` arma `descConstancia(_ubiUltimo, scFC)` —coordenada como cifra grande, fecha y hora, origen y precisión, coberturas por jerarquía, PM, coadministración, zona PM y PGOEDF del punto, área más cercana— y reutiliza el generador de las fichas (`d.grande`, `d.capas` explícitas, `d.soloPunto`).
